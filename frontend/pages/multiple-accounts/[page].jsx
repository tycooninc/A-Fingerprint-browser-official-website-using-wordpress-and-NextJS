...
export const getStaticPaths = async ({locales}) => {
    const posts = await getBlogSubCatIndex('multiple-accounts')
...
export const getStaticProps = async (context) => {
    let posts
    if(context.locale){
        posts = await getBlogSubCatIndex('multiple-accounts', context.locale, context.params.page)
    }
    return {
        props:{
            posts
...
const Page = ({text, posts}) => {
    ...
    return (
        <div className="md:pt-20 mb-32">
            <NextSeo {...SEO}/>
            <Navigation title={'Multiple Accounts'} nav_array={posts.nav_array}/>
            {
                posts?.posts && (
