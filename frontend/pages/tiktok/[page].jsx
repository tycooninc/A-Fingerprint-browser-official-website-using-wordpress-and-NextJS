...
export const getStaticPaths = async ({locales}) => {
    const posts = await getBlogSubCatIndex('tiktok')
...
export const getStaticProps = async (context) => {
    let posts
    if(context.locale){
        posts = await getBlogSubCatIndex('tiktok', context.locale, context.params.page)
    }
    return {
        props:{
            posts
...
const Page = ({text, posts}) => {
    ...
    return (
        <div className="md:pt-20 mb-32">
            <NextSeo {...SEO}/>
            <Navigation title={'TikTok'} nav_array={posts.nav_array}/>
            {
                posts?.posts && (
