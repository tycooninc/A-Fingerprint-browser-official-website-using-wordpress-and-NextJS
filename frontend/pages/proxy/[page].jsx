...
export const getStaticPaths = async ({locales}) => {
    const posts = await getBlogSubCatIndex('proxy')
...
export const getStaticProps = async (context) => {
    let posts
    if(context.locale){
        posts = await getBlogSubCatIndex('proxy', context.locale, context.params.page)
    }
    return {
        props:{
            posts
...
const Page = ({text, posts}) => {
    ...
    return (
        <div className="md:pt-20 mb-32">
            <NextSeo {...SEO}/>
            <Navigation title={'Proxy'} nav_array={posts.nav_array}/>
            {
                posts?.posts && (
