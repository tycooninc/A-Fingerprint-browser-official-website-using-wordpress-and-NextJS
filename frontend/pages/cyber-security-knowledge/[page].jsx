...
export const getStaticPaths = async ({locales}) => {
    const posts = await getBlogSubCatIndex('cyber-security-knowledge')
...
export const getStaticProps = async (context) => {
    let posts
    if(context.locale){
        posts = await getBlogSubCatIndex('cyber-security-knowledge', context.locale, context.params.page)
    }
    return {
        props:{
            posts
...
const Page = ({text, posts}) => {
    ...
    return (
        <div className="md:pt-20 mb-32">
            <NextSeo {...SEO}/>
            <Navigation title={'Cyber Security Knowledge'} nav_array={posts.nav_array}/>
            {
                posts?.posts && (
